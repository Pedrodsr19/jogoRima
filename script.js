...
function renderMcModal(){
  ...
  if(tab==='geral'){
    ...
    cardStyle=buildCardStyle(titleCount,id);
    body=`<div class="statgrid">
      <div class="statbox"><b>${matches.length}</b><span>Batalhas</span></div>
      <div class="statbox"><b>${wins}</b><span>Vitórias</span></div>
      <div class="statbox"><b>${losses}</b><span>Derrotas</span></div>
  ...
  root.innerHTML=`<div class="modalOverlay" onclick="if(event.target===this)closeMcModal()">
    <div class="modalCard" style="${cardStyle}">
      ...
      <div class="editorBox">
        ...
        <h4>Amigos (${m.amigos.length})</h4>
        <button class="btn secondary small" onclick="toggleAmigosPanel('${id}')">${window.__showAmigos[id]?'Ocultar':'Editar amigos'}</button>
        ${window.__showAmigos[id]?`<div class="mcRelList">${db.mcs.filter(x=>x.id!==id).map(x=>`<label class="checkline"><input type="checkbox" ${m.amigos.includes(x.id)?'checked':''} onchange="toggleAmigo('${id}','${x.id}')"><span>${esc(x.name)}</span></label>`).join('')||'<p class="muted">Cadastre outros MCs.</p>'}</div>`:''}
        <h4>Rivais (${m.rivais.length})</h4>
        <button class="btn secondary small" onclick="toggleRivaisPanel('${id}')">${window.__showRivais[id]?'Ocultar':'Editar rivais'}</button>
        ${window.__showRivais[id]?`<div class="mcRelList">${db.mcs.filter(x=>x.id!==id).map(x=>`<label class="checkline"><input type="checkbox" ${m.rivais.includes(x.id)?'checked':''} onchange="toggleRival('${id}','${x.id}')"><span>${esc(x.name)}</span></label>`).join('')||'<p class="muted">Cadastre outros MCs.</p>'}</div>`:''}
      </div>
    </div>
  </div>`;
}
window.__showAmigos={}; window.__showRivais={};
function toggleAmigosPanel(id){ window.__showAmigos[id]=!window.__showAmigos[id]; renderMcModal(); }
function toggleRivaisPanel(id){ window.__showRivais[id]=!window.__showRivais[id]; renderMcModal(); }
function toggleAmigo(mcId,otherId){
  const m=mcById(mcId); if(!m) return;
  if(m.amigos.includes(otherId)) m.amigos=m.amigos.filter(x=>x!==otherId);
  else { m.amigos.push(otherId); m.rivais=m.rivais.filter(x=>x!==otherId); }
  save(); renderMcModal();
}
function toggleRival(mcId,otherId){
  const m=mcById(mcId); if(!m) return;
  if(m.rivais.includes(otherId)) m.rivais=m.rivais.filter(x=>x!==otherId);
  else { m.rivais.push(otherId); m.amigos=m.amigos.filter(x=>x!==otherId); }
  save(); renderMcModal();
}
...
function resetNacional(){
  ...
  db.mcs.forEach(m=>{
    const live=collectNationalMatchesForMc(m.id);
    if(live.length){
      db.nationalStats[m.id]=db.nationalStats[m.id]||{wins:0,losses:0};
      db.nationalStats[m.id].wins+=live.filter(x=>x.won).length;
      db.nationalStats[m.id].losses+=live.filter(x=>!x.won).length;
    }
  });
  const newStates={};
...
function simRegional(estado,idx,mode){
  ...
  if(isBracketComplete(reg.bracket) && !reg.titleLogged){
    ...
    const sd2=getStateData(estado);
    celebrateChampion(mcName(champ),'Campeão Regional '+idx+' - '+estado, sd2.color||'#2f6bff', sd2.color2||'#ff2d4d', champ, false);
  }
  save(); render();
}
...
function simEstadual(estado,mode){
  ...
  if(mode==='oneround') simulateSingleRound(sd.estadualBracket,2);
...
function simNacional(mode){
  const nac=db.national.nacional;
  if(mode==='oneround') simulateSingleRound(nac.prefaseBracket,2);
...
function simNacionalMain(mode){
  const nac=db.national.nacional;
  if(mode==='oneround') simulateSingleRound(nac.mainBracket,2);
...
function sideNamesHtml(bracket,id){
  ...
  if(p.kind==='mc') return `<span class="nm">${mcLinkAv(p.id,p.name)}</span>`;
  return p.mcIds.map(mid=>`<span class="nm">${mcLinkAv(mid)}</span>`).join('');
}
...
function roundModeLabel(mode,teamSize){
  if(mode==='team'){ if(teamSize===2)return 'Dupla'; if(teamSize===3)return 'Trio'; if(teamSize===4)return 'Quarteto'; return 'Solo'; }
  return teamSize>1?'Decisivo (solo)':'Solo';
}
function matchRoundsHtml(bracket,m){
  if(!m.rounds||!m.rounds.length) return '';
  const pa=bracket.participants[m.a], pb=bracket.participants[m.b];
  const teamSize=pa&&pa.kind==='team'?pa.mcIds.length:1;
  const rows=m.rounds.map((r,i)=>{
    const aLbl=r.mode==='team'?(pa.kind==='mc'?mcLink(pa.id,pa.name):teamNameHtml(pa)):mcLink(r.repA);
    const bLbl=r.mode==='team'?(pb.kind==='mc'?mcLink(pb.id,pb.name):teamNameHtml(pb)):mcLink(r.repB);
    const winLbl=r.winSide==='a'?aLbl:bLbl;
    return `<div class="roundRow"><span class="roundTag">R${i+1} · ${roundModeLabel(r.mode,teamSize)}</span> ${aLbl} <b>vs</b> ${bLbl} — venceu ${winLbl}</div>`;
  }).join('');
  return `<details class="roundDetails"><summary>Ver rounds (${m.rounds.length})</summary>${rows}</details>`;
}
function matchHasRivalry(bracket,m){
  if(!m.a||!m.b) return false;
  const pa=bracket.participants[m.a], pb=bracket.participants[m.b]; if(!pa||!pb) return false;
  const idsA=pa.kind==='team'?pa.mcIds:[pa.id];
  const idsB=pb.kind==='team'?pb.mcIds:[pb.id];
  return idsA.some(a=>{ const ma=mcById(a); return ma&&idsB.some(b=>ma.rivais.includes(b)); });
}
function matchHTML(bracket,m){
  const scoreTxt=m.done?`<div class="vs-score">${m.scoreA} × ${m.scoreB}</div>`:(m.scoreA||m.scoreB?`<div class="vs-score">${m.scoreA||0} × ${m.scoreB||0}</div>`:`<div class="vs-score vs-pending">VS</div>`);
  const rivalTag=matchHasRivalry(bracket,m)?`<div class="rivalTag">🔥 Rivalidade</div>`:'';
  return `<div class="match ${m.done?'done':''}">${rivalTag}${sideHTML(bracket,m.a,m.winner,m.done)}${scoreTxt}${sideHTML(bracket,m.b,m.winner,m.done)}${matchRoundsHtml(bracket,m)}</div>`;
}
function renderBracketBlock(bracket,targetWins,fnName,args){
  ...
  ${complete?`<div class="badge">Campeão: ${bracket.participants[champ]?(bracket.participants[champ].kind==='mc'?mcLinkAv(champ,bracket.participants[champ].name):bracket.participants[champ].mcIds.map(mid=>mcLinkAv(mid)).join(' & ')):''}</div>`:
  `<div class="actionsrow">
    <button class="btn small" onclick="${call('oneround')}">Simular 1 Round</button>
    <button class="btn small secondary" onclick="${call('one')}">Simular Batalha</button>
...
function renderFlatBlock(bracket,fn){
  ...
  `<div class="actionsrow">
    <button class="btn small" onclick="${fn}('oneround')">Simular 1 Round</button>
    <button class="btn small secondary" onclick="${fn}('one')">Simular Batalha</button>
...
function viewBattleDetail(id){
  ...
  const titleRows=titleRanking.map((r,i)=>`<div class="rankrow"><div><span class="pos">${i+1}º</span> ${mcLinkAv(r[0])}</div><b>${r[1]} título(s)</b></div>`).join('');
  const campeoes=ultimosCampeoes(b);
  const campHtml=campeoes.length?campeoes.map(c=>`<div class="champrow"><span class="badge">${esc(c.edition)}</span> ${c.label.kind==='mc'?mcLinkAv(c.label.ids[0],c.label.name):c.label.ids.map(mid=>mcLinkAv(mid)).join(' & ')}</div>`).join(''):'<p class="muted">Nenhuma edição concluída ainda.</p>';
...
function finalizarTemporada(battleId){
  ...
  celebrateChampion(mcName(championMcId),'Campeão da Temporada '+b.currentSeason+' - '+b.name, b.color, b.color2, championMcId, false);
  b.ranking={};
  b.currentSeason+=1;
  save(); render();
}
...
function weightedDraft(battle,needed){
  ...
  if(battle.estado){
    const factor=battle.tipo==='underground'?0.5:0.8;
    ids.forEach(id=>{ const m=mcById(id); if(m && m.estado!==battle.estado){ weight[id]=weight[id]*factor; } });
  }
...
function createEdition(battleId){
  ...
  if(teamSize===1){
    ...
    const groups=formTeams(pool,teamSize);
  ...
  const duplaTipo=fmt==='dupla'?(Math.random()<0.5?'tipo1':'tipo2'):undefined;
  const edition={id:uid(),name,format:fmt,formatLabel,size,teamSize,targetWins,duplaTipo,bracket,status:'drawn',pointsApplied:false,excludeFromRanking:!!document.getElementById('edNoRanking').checked};
...
function simEdition(battleId,edId,mode){
  ...
  applyRankingIfNeeded(b,ed);
  if(isBracketComplete(ed.bracket) && !ed.edCelebrated){
    ed.edCelebrated=true;
    const champ=bracketChampion(ed.bracket);
    const p=ed.bracket.participants[champ];
    if(p) celebrateChampion(p.kind==='mc'?p.name:p.mcIds.map(mid=>mcName(mid)).join(' & '),'Campeão - '+ed.name+' ('+b.name+')', b.color, b.color2, p.kind==='mc'?p.id:null, false);
  }
  save(); render();
}
...
function createEventEdition(eventId){
  ...
  const bracket=buildBracket(shuffle(participants));
  const duplaTipo=fmt==='dupla'?(Math.random()<0.5?'tipo1':'tipo2'):undefined;
  ev.editions.push({id:uid(),name,format:fmt,formatLabel,size,teamSize,targetWins,duplaTipo,bracket,status:'drawn'});
...
function simEventEdition(eventId,edId,mode){
  ...
  if(mode==='phase') simulatePhase(ed.bracket,ed.targetWins,ed.duplaTipo);
  if(mode==='all') simulateAll(ed.bracket,ed.targetWins,ed.duplaTipo);
  if(isBracketComplete(ed.bracket) && !ed.celebrated){
    ed.celebrated=true;
    const champ=bracketChampion(ed.bracket);
    const p=ed.bracket.participants[champ];
...
function runWeeklyEdition(b,format,mcId,forceInclude){
  ...
  let participants;
  if(teamSize===1){ participants=pool.map(mcParticipant); }
  else { const groups=formTeams(pool,teamSize); participants=groups.map(g=>{ const mcs=g.map(mcById); const level=Math.round(mcs.reduce((s,mm)=>s+mm.nivel,0)/mcs.length); return {id:uid(),kind:'team',name:mcs.map(mm=>mm.name).join(' & '),level,mcIds:g}; }); }
  const bracket=buildBracket(shuffle(participants));
...
function viewCareerHome(){
  ...
  <div class="content">
    <div class="list-grid">${rows||'<p class="muted">Nenhuma carreira criada.</p>'}</div>
    ${db.careers.length<3?`<div class="card"><button class="btn" onclick="nav('career/create')">Criar Carreira</button></div>`:'<p class="muted">Máximo de 3 carreiras atingido.</p>'}
  </div>`;
}
function viewCareerCreate(){
  return `${topbar('Nova Carreira','','career')}
  <div class="content"><div class="card">
    <label>Nome</label><input id="careerName" placeholder="Seu nome de MC">
    <label>Estado</label><input id="careerEstado" placeholder="Ex: SP">
    <label>Foto (opcional)</label><input id="careerFoto" placeholder="https://...">
    <label>ou enviar arquivo</label><input type="file" accept="image/*" onchange="handleMcPhotoFile(this,'careerFoto')">
    <button class="btn" onclick="createCareer()">Começar Carreira</button>
  </div></div>`;
}
function createCareer(){
  const name=document.getElementById('careerName').value.trim();
  const estado=document.getElementById('careerEstado').value.trim();
  const foto=document.getElementById('careerFoto').value.trim();
  if(!name||!estado) return alert('Preencha nome e estado.');
...
function viewCareerDashboard(careerId){
  ...
  if(car.pendingInvites.length){
    ...
    return `${topbar('Convite!','',null)}<div class="content"><div class="card">
      ...
      <div class="actionsrow">
        <button class="btn" onclick="respondInvite('${car.id}',0,true)">Aceitar</button>
        <button class="btn secondary" onclick="respondInvite('${car.id}',0,false)">Recusar</button>
      </div>
    </div></div>`;
  }
...
function viewCareerBattle(careerId,battleId){
  ...
  <div class="content">
    ...
    <div class="card"><h3>Últimas Edições</h3>${edRows||'<p class="muted">Nenhuma.</p>'}</div>
